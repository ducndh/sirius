...
CREATE OR REPLACE TABLE queries AS SELECT id AS qid, vec::FLOAT[128] AS qvec
  FROM read_parquet('/var/tmp/vj/data/parquet/sift-128-euclidean_query.parquet') LIMIT 1000;
...
.print === TIMED vss_join 1k x 1M k=10 (run 1 of 2)
.mode trash
.timer on
SELECT left_tbl.qid, right_tbl.iid FROM vss_join(queries, items, qvec, ivec, 10);
.print === TIMED vss_join 1k x 1M k=10 (run 2 of 2)
SELECT left_tbl.qid, right_tbl.iid FROM vss_join(queries, items, qvec, ivec, 10);
.timer off
.mode duckbox
...
WITH h AS (SELECT count(*) AS hits FROM ans a JOIN gt10 g ON g.query_id=a.qid AND g.neighbor_id=a.iid)
SELECT (SELECT count(*) FROM ans) AS rows, h.hits, round(h.hits::DOUBLE/10000, 5) AS recall_at_10 FROM h;
