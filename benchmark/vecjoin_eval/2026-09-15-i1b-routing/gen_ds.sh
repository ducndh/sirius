...
set -euo pipefail
OUT=${OUT:-/var/tmp/vj/i1b_${DS}}
DATA=/var/tmp/vj/data/parquet
DS=${DS:-sift-128-euclidean}; DIM=${DIM:-128}
OUT=${OUT:-/var/tmp/vj/i1b_$DS}
mkdir -p "$OUT"
...
echo "CREATE TABLE base  AS SELECT id, vec::FLOAT[$DIM] AS vec FROM read_parquet('$DATA/${DS}_base.parquet');"
echo "CREATE TABLE query AS SELECT id, vec::FLOAT[$DIM] AS vec FROM read_parquet('$DATA/${DS}_query.parquet');"
echo "CREATE TABLE gt    AS SELECT * FROM read_parquet('$DATA/${DS}_gt.parquet');"
echo "CHECKPOINT;"
...
echo "SELECT tag, rows, hits, round(hits::DOUBLE / (10 * (SELECT count(*) FROM query)), 5) AS recall_at_10 FROM recall;"
echo ".output"
} > "$OUT/pareto.sql"
