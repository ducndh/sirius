...
import os
DS = os.environ.get("DS", "sift-128-euclidean")
K = 10


def load(name):
    t = pq.read_table(f"{DATA}/{DS}_{name}.parquet")
    return np.ascontiguousarray(
        np.stack(t.column("vec").to_numpy(zero_copy_only=False)).astype(np.float32))


def ground_truth():
    t = pq.read_table(f"{DATA}/{DS}_gt.parquet")
...
def main():
    ...
    with open("/var/tmp/vj/i1b_" + DS + "/cuvs.json", "w") as f:
        json.dump(rows, f, indent=1)
    print("\nwrote /var/tmp/vj/i1b_" + DS + "/cuvs.json")
