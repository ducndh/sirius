...
class sirius_scan_manager {
 ...
 private:
  ...
  /// \brief Factory for the duckdb-native scan path. Mirrors create_provider_for
  ///        for parquet. Will fold into the polymorphic info->make_provider(*this)
  ///        dispatch once duckdb_native_scan_operator stores
  ///        unique_ptr<scan_info> instead of duckdb_native_scan_info by value.
  std::unique_ptr<split_provider> create_provider_for(
