...
// DuckDB .db file layout:
//   [0, FILE_HEADER_SIZE) — file header (4096 bytes, two database headers + magic)
...
//     block_start  = FILE_HEADER_SIZE + block_id * alloc
//
                                               ...
                                               duckdb::block_id_t block_id)
{
  ...
  return duckdb::Storage::FILE_HEADER_SIZE + static_cast<std::size_t>(block_id) * alloc +
         static_cast<std::size_t>(bm.GetBlockHeaderSize());
}
